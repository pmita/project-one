// STYLES
import styles from './styles.module.css'

export const Features = async () => {
  return (
    <>
      <h1 className={`${styles.header}`}>What we offer?</h1>
      <section className={`${styles.section}`}>
        <div className={`${styles.feature} ${styles.vertical}`}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="50px"
            height="50px"
            className="primary"
          >
            <path
              d="M22.428.013c-.103.01-.432.042-.727.066C14.883.693 8.497 4.37 4.453 10.024A23.755 23.755 0 0 0 .216 20.51C.023 21.828 0 22.217 0 24.005c0 1.787.023 2.177.216 3.495 1.304 9.012 7.718 16.584 16.417 19.39 1.558.501 3.2.844 5.068 1.05.727.08 3.87.08 4.598 0 3.224-.356 5.954-1.154 8.648-2.529.413-.21.492-.267.436-.314-.038-.028-1.797-2.388-3.909-5.24l-3.838-5.184-4.809-7.117c-2.646-3.913-4.824-7.112-4.842-7.112-.019-.005-.038 3.157-.047 7.018-.014 6.76-.019 7.033-.103 7.192-.122.23-.216.324-.413.427-.15.075-.282.09-.99.09h-.812l-.216-.137a.878.878 0 0 1-.314-.342l-.099-.212.01-9.406.014-9.41.145-.184c.075-.098.235-.225.347-.286.193-.094.268-.103 1.08-.103.957 0 1.116.038 1.365.31.07.075 2.674 3.997 5.79 8.721 3.115 4.724 7.376 11.175 9.469 14.342l3.8 5.756.193-.127c1.703-1.107 3.505-2.683 4.93-4.325a23.89 23.89 0 0 0 5.65-12.268c.193-1.318.216-1.708.216-3.495 0-1.788-.023-2.177-.216-3.495-1.304-9.013-7.718-16.584-16.417-19.39C29.832.623 28.199.28 26.37.074c-.451-.047-3.552-.099-3.942-.061Zm9.825 14.515a.946.946 0 0 1 .474.554c.038.122.047 2.73.038 8.608l-.014 8.436-1.488-2.28-1.492-2.28v-6.132c0-3.964.019-6.193.047-6.3a.957.957 0 0 1 .465-.592c.192-.098.262-.108 1-.108.694 0 .816.01.97.094Z"
            />
          </svg>
          <p className={`${styles.title}`}>Tiling</p>
          <p className={`${styles.description}`}>Elevate your kitchen, bathroom, or flooring with expert tiling craftsmanship. Our meticulous attention to detail and skilled techniques result in stunning tiled surfaces that marry durability with beauty. Whether it's intricate mosaic patterns or sleek modern designs, our tiling solutions bring elegance and sophistication to your space.</p>
        </div>
        <div className={`${styles.feature}`}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="50px"
            height="50px"
            className="primary"
          >
            <path
              d="M22.428.013c-.103.01-.432.042-.727.066C14.883.693 8.497 4.37 4.453 10.024A23.755 23.755 0 0 0 .216 20.51C.023 21.828 0 22.217 0 24.005c0 1.787.023 2.177.216 3.495 1.304 9.012 7.718 16.584 16.417 19.39 1.558.501 3.2.844 5.068 1.05.727.08 3.87.08 4.598 0 3.224-.356 5.954-1.154 8.648-2.529.413-.21.492-.267.436-.314-.038-.028-1.797-2.388-3.909-5.24l-3.838-5.184-4.809-7.117c-2.646-3.913-4.824-7.112-4.842-7.112-.019-.005-.038 3.157-.047 7.018-.014 6.76-.019 7.033-.103 7.192-.122.23-.216.324-.413.427-.15.075-.282.09-.99.09h-.812l-.216-.137a.878.878 0 0 1-.314-.342l-.099-.212.01-9.406.014-9.41.145-.184c.075-.098.235-.225.347-.286.193-.094.268-.103 1.08-.103.957 0 1.116.038 1.365.31.07.075 2.674 3.997 5.79 8.721 3.115 4.724 7.376 11.175 9.469 14.342l3.8 5.756.193-.127c1.703-1.107 3.505-2.683 4.93-4.325a23.89 23.89 0 0 0 5.65-12.268c.193-1.318.216-1.708.216-3.495 0-1.788-.023-2.177-.216-3.495-1.304-9.013-7.718-16.584-16.417-19.39C29.832.623 28.199.28 26.37.074c-.451-.047-3.552-.099-3.942-.061Zm9.825 14.515a.946.946 0 0 1 .474.554c.038.122.047 2.73.038 8.608l-.014 8.436-1.488-2.28-1.492-2.28v-6.132c0-3.964.019-6.193.047-6.3a.957.957 0 0 1 .465-.592c.192-.098.262-.108 1-.108.694 0 .816.01.97.094Z"
            />
          </svg>
          <p className={`${styles.title}`}>Painting</p>
          <p className={`${styles.description}`}>Transform your space with precision and expertise. From refreshing walls with vibrant hues to intricate detailing, our professional painting services breathe new life into any room.</p>
        </div>
        <div className={`${styles.feature}`}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="50px"
            height="50px"
            className="primary"
          >
            <path
              d="M22.428.013c-.103.01-.432.042-.727.066C14.883.693 8.497 4.37 4.453 10.024A23.755 23.755 0 0 0 .216 20.51C.023 21.828 0 22.217 0 24.005c0 1.787.023 2.177.216 3.495 1.304 9.012 7.718 16.584 16.417 19.39 1.558.501 3.2.844 5.068 1.05.727.08 3.87.08 4.598 0 3.224-.356 5.954-1.154 8.648-2.529.413-.21.492-.267.436-.314-.038-.028-1.797-2.388-3.909-5.24l-3.838-5.184-4.809-7.117c-2.646-3.913-4.824-7.112-4.842-7.112-.019-.005-.038 3.157-.047 7.018-.014 6.76-.019 7.033-.103 7.192-.122.23-.216.324-.413.427-.15.075-.282.09-.99.09h-.812l-.216-.137a.878.878 0 0 1-.314-.342l-.099-.212.01-9.406.014-9.41.145-.184c.075-.098.235-.225.347-.286.193-.094.268-.103 1.08-.103.957 0 1.116.038 1.365.31.07.075 2.674 3.997 5.79 8.721 3.115 4.724 7.376 11.175 9.469 14.342l3.8 5.756.193-.127c1.703-1.107 3.505-2.683 4.93-4.325a23.89 23.89 0 0 0 5.65-12.268c.193-1.318.216-1.708.216-3.495 0-1.788-.023-2.177-.216-3.495-1.304-9.013-7.718-16.584-16.417-19.39C29.832.623 28.199.28 26.37.074c-.451-.047-3.552-.099-3.942-.061Zm9.825 14.515a.946.946 0 0 1 .474.554c.038.122.047 2.73.038 8.608l-.014 8.436-1.488-2.28-1.492-2.28v-6.132c0-3.964.019-6.193.047-6.3a.957.957 0 0 1 .465-.592c.192-.098.262-.108 1-.108.694 0 .816.01.97.094Z"
            />
          </svg>
          <p className={`${styles.title}`}>Renovation</p>
          <p className={`${styles.description}`}>Reimagine your space with our expert renovation services. Whether it’s updating a single room or a complete home makeover, we bring your vision to life with meticulous craftsmanship and attention to detail.</p>
        </div>
        <div className={`${styles.feature} ${styles.horizontal}`}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="50px"
            height="50px"
            className="primary"
          >
            <path
              d="M22.428.013c-.103.01-.432.042-.727.066C14.883.693 8.497 4.37 4.453 10.024A23.755 23.755 0 0 0 .216 20.51C.023 21.828 0 22.217 0 24.005c0 1.787.023 2.177.216 3.495 1.304 9.012 7.718 16.584 16.417 19.39 1.558.501 3.2.844 5.068 1.05.727.08 3.87.08 4.598 0 3.224-.356 5.954-1.154 8.648-2.529.413-.21.492-.267.436-.314-.038-.028-1.797-2.388-3.909-5.24l-3.838-5.184-4.809-7.117c-2.646-3.913-4.824-7.112-4.842-7.112-.019-.005-.038 3.157-.047 7.018-.014 6.76-.019 7.033-.103 7.192-.122.23-.216.324-.413.427-.15.075-.282.09-.99.09h-.812l-.216-.137a.878.878 0 0 1-.314-.342l-.099-.212.01-9.406.014-9.41.145-.184c.075-.098.235-.225.347-.286.193-.094.268-.103 1.08-.103.957 0 1.116.038 1.365.31.07.075 2.674 3.997 5.79 8.721 3.115 4.724 7.376 11.175 9.469 14.342l3.8 5.756.193-.127c1.703-1.107 3.505-2.683 4.93-4.325a23.89 23.89 0 0 0 5.65-12.268c.193-1.318.216-1.708.216-3.495 0-1.788-.023-2.177-.216-3.495-1.304-9.013-7.718-16.584-16.417-19.39C29.832.623 28.199.28 26.37.074c-.451-.047-3.552-.099-3.942-.061Zm9.825 14.515a.946.946 0 0 1 .474.554c.038.122.047 2.73.038 8.608l-.014 8.436-1.488-2.28-1.492-2.28v-6.132c0-3.964.019-6.193.047-6.3a.957.957 0 0 1 .465-.592c.192-.098.262-.108 1-.108.694 0 .816.01.97.094Z"
            />
          </svg>
          <p className={`${styles.title}`}>Miscelleneous</p>
          <p className={`${styles.description}`}>Enhance your outdoor spaces with our versatile services. From sturdy brickwork to impeccably landscaped gardens, we offer comprehensive solutions tailored to your needs. Whether it's reviving your driveway, creating an oasis of greenery, or securing your property with durable fencing, our expertise ensures impeccable results that stand the test of time.</p>
        </div>
      </section>
    </>
  )
}